...
MAJOR_VERSION=2
MINOR_VERSION=13
rm -f libcfint*.gz *.deb *.build *.dsc *.log *.changes ex.cmd
pushd libcfint
	if [ -f Makefile ]; then
		make distclean
	fi
...
pushd libcfint
	...
	if [ ! -d /usr/include/msobkow/${MAJOR_VERSION}.0.${MINOR_VERSION}/cfint ]; then
		sudo mkdir /usr/include/msobkow/${MAJOR_VERSION}.0.${MINOR_VERSION}/cfint
	fi
	sudo chmod 755 /usr/include/msobkow/${MAJOR_VERSION}.0.${MINOR_VERSION}/cfint
	if [ ! -d /usr/include/msobkow/${MAJOR_VERSION}.0.${MINOR_VERSION}/cfintobj ]; then
		sudo mkdir /usr/include/msobkow/${MAJOR_VERSION}.0.${MINOR_VERSION}/cfintobj
	fi
	sudo chmod 755 /usr/include/msobkow/${MAJOR_VERSION}.0.${MINOR_VERSION}/cfintobj
	autoreconf -fi
	aclocal
	autoconf
	automake
	...
	sudo chmod 755 /usr/include/msobkow//${MAJOR_VERSION}.0.${MINOR_VERSION}
	sudo chmod 755 /usr/include/msobkow//${MAJOR_VERSION}.0.${MINOR_VERSION}/cfint
	sudo sh -c "cd /usr/include/msobkow//${MAJOR_VERSION}.0.${MINOR_VERSION}/cfint && chmod 644 *.hpp"
	sudo chmod 755 /usr/include/msobkow//${MAJOR_VERSION}.0.${MINOR_VERSION}/cfintobj
	sudo sh -c "cd /usr/include/msobkow//${MAJOR_VERSION}.0.${MINOR_VERSION}/cfintobj && chmod 644 *.hpp"
