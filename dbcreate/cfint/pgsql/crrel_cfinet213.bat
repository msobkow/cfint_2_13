@ECHO off
REM
REM	@(#) dbcreate/cfint/pgsql/crrel_cfinet213.bat
REM
...
psql -h %pghost% -d %pgdb% -U %pguser% -a -e -f crrel_mjvrdef.pgsql >>%logfilename% 2>&1
psql -h %pghost% -d %pgdb% -U %pguser% -a -e -f crrel_mimetype.pgsql >>%logfilename% 2>&1
psql -h %pghost% -d %pgdb% -U %pguser% -a -e -f crrel_mnvrdef.pgsql >>%logfilename% 2>&1
