#!/bin/bash
#
#	@(#) dbcreate/cfint/pgsql/crtypes_cfinet213.bash
#
...
psql -h ${pghost} ${PGPORT} -d ${pgdb} -U ${pguser} -a -e -f crtype_cfinet213_cc_rec.pgsql >>$logfilename 2>&1
...
psql -h ${pghost} ${PGPORT} -d ${pgdb} -U ${pguser} -a -e -f crtype_mjvrdef_rec.pgsql >>$logfilename 2>&1

psql -h ${pghost} ${PGPORT} -d ${pgdb} -U ${pguser} -a -e -f crtype_mimetype_rec.pgsql >>$logfilename 2>&1

psql -h ${pghost} ${PGPORT} -d ${pgdb} -U ${pguser} -a -e -f crtype_mnvrdef_rec.pgsql >>$logfilename 2>&1
