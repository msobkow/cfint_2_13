// Description: Java 11 XMsg Client DbIO implementation for MajorVersion.
...
/*
 *	CFIntXMsgClntMajorVersion XMsg Client DbIO implementation
 *	for MajorVersion.
 */
public class CFIntXMsgClntMajorVersion
	implements ICFIntMajorVersionTable
{
	private CFIntXMsgClntSchema schema;

	public CFIntXMsgClntMajorVersion( CFIntXMsgClntSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFIntMajorVersionBuff Buff )
	{
		...
		String rqst = CFIntXMsgSchemaMessageFormatter.formatRqstXmlPreamble() + "\n"
			+	"\t" + CFIntXMsgMajorVersionMessageFormatter.formatMajorVersionRqstCreate( "\n\t\t\t", Buff )
			+	"\n"
		...
		if( lastObjectProcessed != null ) {
			ICFIntMajorVersionObj realised = null;
			if( lastObjectProcessed instanceof ICFIntMajorVersionObj ) {
				realised = (ICFIntMajorVersionObj)lastObjectProcessed;
			}
			else {
			...
			if( realised != null ) {
				Buff.set( realised.getMajorVersionBuff() );
			}
		}
		else {
		...
		CFIntMajorVersionPKey PKey )
	{
		...
		String rqst = CFIntXMsgSchemaMessageFormatter.formatRqstXmlPreamble() + "\n"
			+	"\t" + CFIntXMsgMajorVersionMessageFormatter.formatMajorVersionRqstRead( "\n\t\t\t", PKey )
			+	"\n"
			+	CFIntXMsgSchemaMessageFormatter.formatRqstXmlPostamble();
		try {
			schema.getCFTipClientHandler().issueAppRequest( rqst );
		...
		if( lastObjectProcessed != null ) {
			...
			if( realised != null ) {
				buff = realised.getMajorVersionBuff();
			}
		}
		return( buff );
	}
		...
		CFIntMajorVersionPKey PKey )
	{
		...
		String rqst = CFIntXMsgSchemaMessageFormatter.formatRqstXmlPreamble() + "\n"
			+	"\t" + CFIntXMsgMajorVersionMessageFormatter.formatMajorVersionRqstLock( "\n\t\t\t", PKey )
			+	"\n"
			+	CFIntXMsgSchemaMessageFormatter.formatRqstXmlPostamble();
		try {
			schema.getCFTipClientHandler().issueAppRequest( rqst );
	...
	public CFIntMajorVersionBuff[] readAllDerived( CFSecAuthorization Authorization ) {
		...
		String rqst = CFIntXMsgSchemaMessageFormatter.formatRqstXmlPreamble() + "\n"
			+	"\t" + CFIntXMsgMajorVersionMessageFormatter.formatMajorVersionRqstReadAll( "\n\t\t\t" )
			+	"\n"
			+	CFIntXMsgSchemaMessageFormatter.formatRqstXmlPostamble();
		try {
			schema.getCFTipClientHandler().issueAppRequest( rqst );
		...
		List<ICFIntMajorVersionObj> sortedList =
			(List<ICFIntMajorVersionObj>)sortedListObj;
		CFIntMajorVersionBuff arr[];
		if( sortedList != null ) {
			int sz = sortedList.size();
			arr = new CFIntMajorVersionBuff[ sz ];
			Iterator<ICFIntMajorVersionObj> iter = sortedList.iterator();
			ICFIntMajorVersionObj cur;
			for( int idx = 0; idx < sz; idx ++ ) {
				cur = (ICFIntMajorVersionObj)iter.next();
				arr[idx] = cur.getMajorVersionBuff();
			}
		}
		else {
			arr = new CFIntMajorVersionBuff[ 0 ];
		}
		return( arr );
	}
		...
		long argId )
	{
		...
		String rqst = CFIntXMsgSchemaMessageFormatter.formatRqstXmlPreamble() + "\n"
			+	"\t" + CFIntXMsgMajorVersionMessageFormatter.formatMajorVersionRqstRdByIdIdx( "\n\t\t\t",
							argTenantId,
							argId )
			+	"\n"
			+	CFIntXMsgSchemaMessageFormatter.formatRqstXmlPostamble();
		...
		long argTenantId )
	{
		...
		String rqst = CFIntXMsgSchemaMessageFormatter.formatRqstXmlPreamble() + "\n"
			+	"\t" + CFIntXMsgMajorVersionMessageFormatter.formatMajorVersionRqstRdByTenantIdx( "\n\t\t\t",
							argTenantId )
			+	"\n"
			+	CFIntXMsgSchemaMessageFormatter.formatRqstXmlPostamble();
		try {
		...
		long argSubProjectId )
	{
		...
		String rqst = CFIntXMsgSchemaMessageFormatter.formatRqstXmlPreamble() + "\n"
			+	"\t" + CFIntXMsgMajorVersionMessageFormatter.formatMajorVersionRqstRdBySubProjectIdx( "\n\t\t\t",
							argTenantId,
							argSubProjectId )
			+	"\n"
			+	CFIntXMsgSchemaMessageFormatter.formatRqstXmlPostamble();
		...
		String argName )
	{
		...
		String rqst = CFIntXMsgSchemaMessageFormatter.formatRqstXmlPreamble() + "\n"
			+	"\t" + CFIntXMsgMajorVersionMessageFormatter.formatMajorVersionRqstRdByNameIdx( "\n\t\t\t",
							argTenantId,
							argSubProjectId,
							argName )
			+	"\n"
		...
		CFIntMajorVersionBuff Buff )
	{
		...
		String rqst = CFIntXMsgSchemaMessageFormatter.formatRqstXmlPreamble() + "\n"
			+	"\t" + CFIntXMsgMajorVersionMessageFormatter.formatMajorVersionRqstUpdate( "\n\t\t\t", Buff )
			+	"\n"
			+	CFIntXMsgSchemaMessageFormatter.formatRqstXmlPostamble();
		try {
			schema.getCFTipClientHandler().issueAppRequest( rqst );
		...
		CFIntMajorVersionBuff Buff )
	{
		...
		String rqst = CFIntXMsgSchemaMessageFormatter.formatRqstXmlPreamble() + "\n"
			+	"\t" + CFIntXMsgMajorVersionMessageFormatter.formatMajorVersionRqstDelete( "\n\t\t\t", Buff )
			+	"\n"
			+	CFIntXMsgSchemaMessageFormatter.formatRqstXmlPostamble();
		try {
			schema.getCFTipClientHandler().issueAppRequest( rqst );
		...
		long argId )
	{
		...
		String rqst = CFIntXMsgSchemaMessageFormatter.formatRqstXmlPreamble() + "\n"
			+	"\t" + CFIntXMsgMajorVersionMessageFormatter.formatMajorVersionRqstDelByIdIdx( "\n\t\t\t",
							argTenantId,
							argId )
			+	"\n"
			+	CFIntXMsgSchemaMessageFormatter.formatRqstXmlPostamble();
		...
		long argTenantId )
	{
		...
		String rqst = CFIntXMsgSchemaMessageFormatter.formatRqstXmlPreamble() + "\n"
			+	"\t" + CFIntXMsgMajorVersionMessageFormatter.formatMajorVersionRqstDelByTenantIdx( "\n\t\t\t",
							argTenantId )
			+	"\n"
			+	CFIntXMsgSchemaMessageFormatter.formatRqstXmlPostamble();
		try {
		...
		long argSubProjectId )
	{
		...
		String rqst = CFIntXMsgSchemaMessageFormatter.formatRqstXmlPreamble() + "\n"
			+	"\t" + CFIntXMsgMajorVersionMessageFormatter.formatMajorVersionRqstDelBySubProjectIdx( "\n\t\t\t",
							argTenantId,
							argSubProjectId )
			+	"\n"
			+	CFIntXMsgSchemaMessageFormatter.formatRqstXmlPostamble();
		...
		String argName )
	{
		...
		String rqst = CFIntXMsgSchemaMessageFormatter.formatRqstXmlPreamble() + "\n"
			+	"\t" + CFIntXMsgMajorVersionMessageFormatter.formatMajorVersionRqstDelByNameIdx( "\n\t\t\t",
							argTenantId,
							argSubProjectId,
							argName )
			+	"\n"
