// Description: Java 11 XMsg Client DbIO implementation for Tld.
...
/*
 *	CFIntXMsgClntTld XMsg Client DbIO implementation
 *	for Tld.
 */
public class CFIntXMsgClntTld
	implements ICFIntTldTable
{
	private CFIntXMsgClntSchema schema;

	public CFIntXMsgClntTld( CFIntXMsgClntSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFIntTldBuff Buff )
	{
		...
		String rqst = CFIntXMsgSchemaMessageFormatter.formatRqstXmlPreamble() + "\n"
			+	"\t" + CFIntXMsgTldMessageFormatter.formatTldRqstCreate( "\n\t\t\t", Buff )
			+	"\n"
		...
		if( lastObjectProcessed != null ) {
			ICFIntTldObj realised = null;
			if( lastObjectProcessed instanceof ICFIntTldObj ) {
				realised = (ICFIntTldObj)lastObjectProcessed;
			}
			else {
		...
		CFIntTldPKey PKey )
	{
		...
		String rqst = CFIntXMsgSchemaMessageFormatter.formatRqstXmlPreamble() + "\n"
			+	"\t" + CFIntXMsgTldMessageFormatter.formatTldRqstRead( "\n\t\t\t", PKey )
			+	"\n"
			+	CFIntXMsgSchemaMessageFormatter.formatRqstXmlPostamble();
		try {
			schema.getCFTipClientHandler().issueAppRequest( rqst );
		...
		CFIntTldPKey PKey )
	{
		...
		String rqst = CFIntXMsgSchemaMessageFormatter.formatRqstXmlPreamble() + "\n"
			+	"\t" + CFIntXMsgTldMessageFormatter.formatTldRqstLock( "\n\t\t\t", PKey )
			+	"\n"
			+	CFIntXMsgSchemaMessageFormatter.formatRqstXmlPostamble();
		try {
			schema.getCFTipClientHandler().issueAppRequest( rqst );
	...
	public CFIntTldBuff[] readAllDerived( CFSecAuthorization Authorization ) {
		...
		String rqst = CFIntXMsgSchemaMessageFormatter.formatRqstXmlPreamble() + "\n"
			+	"\t" + CFIntXMsgTldMessageFormatter.formatTldRqstReadAll( "\n\t\t\t" )
			+	"\n"
			+	CFIntXMsgSchemaMessageFormatter.formatRqstXmlPostamble();
		try {
			schema.getCFTipClientHandler().issueAppRequest( rqst );
		...
		List<ICFIntTldObj> sortedList =
			(List<ICFIntTldObj>)sortedListObj;
		CFIntTldBuff arr[];
		if( sortedList != null ) {
			int sz = sortedList.size();
			arr = new CFIntTldBuff[ sz ];
			Iterator<ICFIntTldObj> iter = sortedList.iterator();
			ICFIntTldObj cur;
			for( int idx = 0; idx < sz; idx ++ ) {
				cur = (ICFIntTldObj)iter.next();
				arr[idx] = cur.getTldBuff();
			}
		}
		else {
			arr = new CFIntTldBuff[ 0 ];
		}
		return( arr );
	}
		...
		long argId )
	{
		...
		String rqst = CFIntXMsgSchemaMessageFormatter.formatRqstXmlPreamble() + "\n"
			+	"\t" + CFIntXMsgTldMessageFormatter.formatTldRqstRdByIdIdx( "\n\t\t\t",
							argTenantId,
							argId )
			+	"\n"
			+	CFIntXMsgSchemaMessageFormatter.formatRqstXmlPostamble();
		...
		long argTenantId )
	{
		...
		String rqst = CFIntXMsgSchemaMessageFormatter.formatRqstXmlPreamble() + "\n"
			+	"\t" + CFIntXMsgTldMessageFormatter.formatTldRqstRdByTenantIdx( "\n\t\t\t",
							argTenantId )
			+	"\n"
			+	CFIntXMsgSchemaMessageFormatter.formatRqstXmlPostamble();
		try {
		...
		String argName )
	{
		...
		String rqst = CFIntXMsgSchemaMessageFormatter.formatRqstXmlPreamble() + "\n"
			+	"\t" + CFIntXMsgTldMessageFormatter.formatTldRqstRdByNameIdx( "\n\t\t\t",
							argTenantId,
							argName )
			+	"\n"
			+	CFIntXMsgSchemaMessageFormatter.formatRqstXmlPostamble();
		...
		CFIntTldBuff Buff )
	{
		...
		String rqst = CFIntXMsgSchemaMessageFormatter.formatRqstXmlPreamble() + "\n"
			+	"\t" + CFIntXMsgTldMessageFormatter.formatTldRqstUpdate( "\n\t\t\t", Buff )
			+	"\n"
			+	CFIntXMsgSchemaMessageFormatter.formatRqstXmlPostamble();
		try {
			schema.getCFTipClientHandler().issueAppRequest( rqst );
		...
		CFIntTldBuff Buff )
	{
		...
		String rqst = CFIntXMsgSchemaMessageFormatter.formatRqstXmlPreamble() + "\n"
			+	"\t" + CFIntXMsgTldMessageFormatter.formatTldRqstDelete( "\n\t\t\t", Buff )
			+	"\n"
			+	CFIntXMsgSchemaMessageFormatter.formatRqstXmlPostamble();
		try {
			schema.getCFTipClientHandler().issueAppRequest( rqst );
		...
		long argId )
	{
		...
		String rqst = CFIntXMsgSchemaMessageFormatter.formatRqstXmlPreamble() + "\n"
			+	"\t" + CFIntXMsgTldMessageFormatter.formatTldRqstDelByIdIdx( "\n\t\t\t",
							argTenantId,
							argId )
			+	"\n"
			+	CFIntXMsgSchemaMessageFormatter.formatRqstXmlPostamble();
		...
		long argTenantId )
	{
		...
		String rqst = CFIntXMsgSchemaMessageFormatter.formatRqstXmlPreamble() + "\n"
			+	"\t" + CFIntXMsgTldMessageFormatter.formatTldRqstDelByTenantIdx( "\n\t\t\t",
							argTenantId )
			+	"\n"
			+	CFIntXMsgSchemaMessageFormatter.formatRqstXmlPostamble();
		try {
		...
		String argName )
	{
		...
		String rqst = CFIntXMsgSchemaMessageFormatter.formatRqstXmlPreamble() + "\n"
			+	"\t" + CFIntXMsgTldMessageFormatter.formatTldRqstDelByNameIdx( "\n\t\t\t",
							argTenantId,
							argName )
			+	"\n"
			+	CFIntXMsgSchemaMessageFormatter.formatRqstXmlPostamble();
