// Description: Java 11 MS SQL Server Developer Edition Jdbc DbIO implementation for Tld.
...
/*
 *	CFIntMSSqlTldTable PostgreSQL Jdbc DbIO implementation
 *	for Tld.
 */
public class CFIntMSSqlTldTable
	implements ICFIntTldTable
{
	...
	public CFIntMSSqlTldTable( CFIntMSSqlSchema argSchema ) {
		schema = argSchema;
	}
	...
	public CFIntTldBuff readDerivedByIdIdx( CFSecAuthorization Authorization,
		...
		long Id )
	{
		final String S_ProcName = "CFIntMSSqlTldTable.readDerivedByIdIdx() ";
		CFIntTldBuff buff;
		if( ! schema.isTransactionOpen() ) {
			throw new CFLibUsageException( getClass(),
				S_ProcName,
	...
	public CFIntTldBuff readDerivedByNameIdx( CFSecAuthorization Authorization,
		...
		String Name )
	{
		final String S_ProcName = "CFIntMSSqlTldTable.readDerivedByNameIdx() ";
		CFIntTldBuff buff;
		if( ! schema.isTransactionOpen() ) {
			throw new CFLibUsageException( getClass(),
				S_ProcName,
		...
		long argId )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			String sql = "exec sp_delete_tlddef_by_ididx ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?";
