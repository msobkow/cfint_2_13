...
/*
 *	CFIntMySqlSecSessionTable MySQL Jdbc DbIO implementation
 *	for SecSession.
 */
public class CFIntMySqlSecSessionTable
	implements ICFIntSecSessionTable
{
	...
	public CFIntMySqlSecSessionTable( CFIntMySqlSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFSecSecSessionBuff Buff )
	{
		...
		try {
			...
			stmtCreateByPKey.setString( argIdx++, CFIntMySqlSchema.getTimestampString( Start ) );
			if( Finish != null ) {
				stmtCreateByPKey.setString( argIdx++, CFIntMySqlSchema.getTimestampString( Finish ) );
			}
			else {
				stmtCreateByPKey.setNull( argIdx++, java.sql.Types.VARCHAR );
			}
	...
	throws SQLException
	{
		...
		int idxcol = 1;
		CFSecSecSessionBuff buff = schema.getFactorySecSession().newBuff();		buff.setRequiredSecSessionId( CFIntMySqlSchema.convertUuidString( resultSet.getString( idxcol ) ) );
		idxcol++;
		buff.setRequiredSecUserId( CFIntMySqlSchema.convertUuidString( resultSet.getString( idxcol ) ) );
		idxcol++;
		{
		...
		idxcol++;
		buff.setRequiredStart( CFIntMySqlSchema.convertTimestampString( resultSet.getString( idxcol ) ) );
		idxcol++;
		{
			Calendar colVal = CFIntMySqlSchema.convertTimestampString( resultSet.getString( idxcol ) );
			if( resultSet.wasNull() ) {
		...
		idxcol++;
		{
			UUID colVal = CFIntMySqlSchema.convertUuidString( resultSet.getString( idxcol ) );
			if( resultSet.wasNull() ) {
	...
	public CFSecSecSessionBuff readDerivedByIdIdx( CFSecAuthorization Authorization,
		UUID SecSessionId )
	{
		final String S_ProcName = "CFIntMySqlSecSessionTable.readDerivedByIdIdx() ";
		CFSecSecSessionBuff buff;
		if( ! schema.isTransactionOpen() ) {
			throw new CFLibUsageException( getClass(),
				S_ProcName,
	...
	public CFSecSecSessionBuff readDerivedByStartIdx( CFSecAuthorization Authorization,
		...
		Calendar Start )
	{
		final String S_ProcName = "CFIntMySqlSecSessionTable.readDerivedByStartIdx() ";
		CFSecSecSessionBuff buff;
		if( ! schema.isTransactionOpen() ) {
			throw new CFLibUsageException( getClass(),
				S_ProcName,
		...
		Calendar Start )
	{
		...
		try {
			...
			stmtReadBuffByStartIdx.setString( argIdx++, CFIntMySqlSchema.getTimestampString( Start ) );
			try {
		...
		Calendar Finish )
	{
		...
		try {
			...
			if( Finish != null ) {
				stmtReadBuffByFinishIdx.setString( argIdx++, CFIntMySqlSchema.getTimestampString( Finish ) );
			}
			else {
		...
		UUID priorSecSessionId )
	{
		...
		try {
			...
			if( Finish != null ) {
				stmtPageBuffByFinishIdx.setString( argIdx++, CFIntMySqlSchema.getTimestampString( Finish ) );
			}
			else {
		...
		CFSecSecSessionBuff Buff )
	{
		...
		try {
			...
			stmtUpdateByPKey.setString( argIdx++, CFIntMySqlSchema.getTimestampString( Start ) );
			if( Finish != null ) {
				stmtUpdateByPKey.setString( argIdx++, CFIntMySqlSchema.getTimestampString( Finish ) );
			}
			else {
				stmtUpdateByPKey.setNull( argIdx++, java.sql.Types.VARCHAR );
			}
		...
		Calendar argStart )
	{
		...
		try {
			...
			stmtDeleteByStartIdx.setString( argIdx++, CFIntMySqlSchema.getTimestampString( argStart ) );
		...
		Calendar argFinish )
	{
		...
		try {
			...
			if( argFinish != null ) {
				stmtDeleteByFinishIdx.setString( argIdx++, CFIntMySqlSchema.getTimestampString( argFinish ) );
			}
			else {
