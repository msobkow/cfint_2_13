// Description: Java 11 MySQL Jdbc DbIO implementation for SubProject.
...
/*
 *	CFIntMySqlSubProjectTable MySQL Jdbc DbIO implementation
 *	for SubProject.
 */
public class CFIntMySqlSubProjectTable
	implements ICFIntSubProjectTable
{
	...
	public CFIntMySqlSubProjectTable( CFIntMySqlSchema argSchema ) {
		schema = argSchema;
	}
	...
	public CFIntSubProjectBuff readDerivedByIdIdx( CFSecAuthorization Authorization,
		...
		long Id )
	{
		final String S_ProcName = "CFIntMySqlSubProjectTable.readDerivedByIdIdx() ";
		CFIntSubProjectBuff buff;
		if( ! schema.isTransactionOpen() ) {
			throw new CFLibUsageException( getClass(),
				S_ProcName,
	...
	public CFIntSubProjectBuff readDerivedByNameIdx( CFSecAuthorization Authorization,
		...
		String Name )
	{
		final String S_ProcName = "CFIntMySqlSubProjectTable.readDerivedByNameIdx() ";
		CFIntSubProjectBuff buff;
		if( ! schema.isTransactionOpen() ) {
			throw new CFLibUsageException( getClass(),
				S_ProcName,
		...
		long TopProjectId )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			String sql = "call " + schema.getLowerDbSchemaName() + ".sp_read_sprjdef_by_topprojectidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " )";
		...
		long argTopProjectId )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			String sql = "call " + schema.getLowerDbSchemaName() + ".sp_delete_sprjdef_by_topprojectidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " )";
			...
			stmtDeleteByTopProjectIdx.executeUpdate();
		}
		catch( SQLException e ) {
			throw new CFLibDbException( getClass(),
				S_ProcName,
