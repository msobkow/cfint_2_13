// Description: Java 11 MySQL Jdbc DbIO implementation for TopProject.
...
/*
 *	CFIntMySqlTopProjectTable MySQL Jdbc DbIO implementation
 *	for TopProject.
 */
public class CFIntMySqlTopProjectTable
	implements ICFIntTopProjectTable
{
	...
	public CFIntMySqlTopProjectTable( CFIntMySqlSchema argSchema ) {
		schema = argSchema;
	}
	...
	public CFIntTopProjectBuff readDerivedByIdIdx( CFSecAuthorization Authorization,
		...
		long Id )
	{
		final String S_ProcName = "CFIntMySqlTopProjectTable.readDerivedByIdIdx() ";
		CFIntTopProjectBuff buff;
		if( ! schema.isTransactionOpen() ) {
			throw new CFLibUsageException( getClass(),
				S_ProcName,
	...
	public CFIntTopProjectBuff readDerivedByNameIdx( CFSecAuthorization Authorization,
		...
		String Name )
	{
		final String S_ProcName = "CFIntMySqlTopProjectTable.readDerivedByNameIdx() ";
		CFIntTopProjectBuff buff;
		if( ! schema.isTransactionOpen() ) {
			throw new CFLibUsageException( getClass(),
				S_ProcName,
		...
		long TopDomainId )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			String sql = "call " + schema.getLowerDbSchemaName() + ".sp_read_tprjdef_by_topdomainidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " )";
		...
		long argTopDomainId )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			String sql = "call " + schema.getLowerDbSchemaName() + ".sp_delete_tprjdef_by_topdomainidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " )";
			...
			stmtDeleteByTopDomainIdx.executeUpdate();
		}
		catch( SQLException e ) {
			throw new CFLibDbException( getClass(),
				S_ProcName,
