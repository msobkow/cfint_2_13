// Description: Java 11 XML SAX Loader CLI persisting as Oracle for CFInt.
...
package org.msscf.msscf.cfint.v2_13.cfint.CFIntSaxOracleLdr;
...
import org.msscf.msscf.cfint.v2_13.cfint.CFIntOracle.*;
import org.msscf.msscf.cfint.v2_13.cfint.CFIntSaxLoader.*;

public class CFIntSaxOracleLdr
	extends CFIntSaxLdr
{
	...
	public static void main( String args[] ) {
		final String S_ProcName = "CFIntSaxOracleLdr.main() ";
		initConsoleLog();
		...
		if( numArgs >= 2 ) {
			...
			String cFIntConfigFileName = homeDir.getPath() + File.separator + ".cfintoraclerc";
			...
			if( ! cFIntConfigFile.exists() ) {
				...
				cFIntConfig.setDbPort( 1526 );
				cFIntConfig.setDbDatabase( "CFINet213" );
				cFIntConfig.setDbUserName( "system" );
			...
			Logger httpLogger = Logger.getLogger( CFIntSaxOracleLdr.class );
			httpLogger.setLevel( Level.WARN );

			ICFIntSchema cFIntSchema = new CFIntOracleSchema();
			...
			CFIntSaxLdr cli = new CFIntSaxOracleLdr();
...
//		BasicConfigurator.configure( new ConsoleAppender( layout, "System.out" ) );
	...
	public void evaluateRemainingArgs( String[] args, int consumed ) {
		...
		if( args.length > consumed ) {
			log.message( "CFIntSaxOracleLdr.evaluateRemainingArgs() WARNING No extra arguments are expected for a RAM database instance, but "
