...
package org.msscf.msscf.v2_13.cfint.cfint.CFIntDb2LUW;
...
/*
 *	CFIntDb2LUWSecSessionTable DB/2 LUW Jdbc DbIO implementation
 *	for SecSession.
 */
public class CFIntDb2LUWSecSessionTable
	implements ICFIntSecSessionTable
{
	...
	public CFIntDb2LUWSecSessionTable( CFIntDb2LUWSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFSecSecSessionBuff Buff )
	{
		...
		try {
			...
			stmtCreateByPKey.setString( argIdx++, CFIntDb2LUWSchema.getTimestampString( Start ) );
			if( Finish != null ) {
				stmtCreateByPKey.setString( argIdx++, CFIntDb2LUWSchema.getTimestampString( Finish ) );
			}
			else {
				stmtCreateByPKey.setNull( argIdx++, java.sql.Types.VARCHAR );
			}
	...
	throws SQLException
	{
		...
		int idxcol = 1;
		CFSecSecSessionBuff buff = schema.getFactorySecSession().newBuff();		buff.setRequiredSecSessionId( CFIntDb2LUWSchema.convertUuidString( resultSet.getString( idxcol ) ) );
		idxcol++;
		...
		idxcol++;
		buff.setRequiredStart( CFIntDb2LUWSchema.convertTimestampString( resultSet.getString( idxcol ) ) );
		idxcol++;
		{
			Calendar colVal = CFIntDb2LUWSchema.convertTimestampString( resultSet.getString( idxcol ) );
			if( resultSet.wasNull() ) {
	...
	public CFSecSecSessionBuff readDerivedByIdIdx( CFSecAuthorization Authorization,
		UUID SecSessionId )
	{
		final String S_ProcName = "CFIntDb2LUWSecSessionTable.readDerivedByIdIdx() ";
		CFSecSecSessionBuff buff;
		if( ! schema.isTransactionOpen() ) {
			throw new CFLibUsageException( getClass(),
				S_ProcName,
	...
	public CFSecSecSessionBuff readDerivedByStartIdx( CFSecAuthorization Authorization,
		...
		Calendar Start )
	{
		final String S_ProcName = "CFIntDb2LUWSecSessionTable.readDerivedByStartIdx() ";
		CFSecSecSessionBuff buff;
		if( ! schema.isTransactionOpen() ) {
			throw new CFLibUsageException( getClass(),
				S_ProcName,
		...
		Calendar Start )
	{
		...
		try {
			...
			stmtReadBuffByStartIdx.setString( argIdx++, CFIntDb2LUWSchema.getTimestampString( Start ) );
		...
		Calendar Finish )
	{
		...
		try {
			...
			if( Finish != null ) {
				stmtReadBuffByFinishIdx.setString( argIdx++, CFIntDb2LUWSchema.getTimestampString( Finish ) );
			}
			else {
		...
		UUID priorSecSessionId )
	{
		...
		try {
			...
			if( Finish != null ) {
				stmtPageBuffByFinishIdx.setString( argIdx++, CFIntDb2LUWSchema.getTimestampString( Finish ) );
			}
			else {
		...
		CFSecSecSessionBuff Buff )
	{
		...
		try {
			...
			stmtUpdateByPKey.setString( argIdx++, CFIntDb2LUWSchema.getTimestampString( Start ) );
			if( Finish != null ) {
				stmtUpdateByPKey.setString( argIdx++, CFIntDb2LUWSchema.getTimestampString( Finish ) );
			}
			else {
				stmtUpdateByPKey.setNull( argIdx++, java.sql.Types.VARCHAR );
			}
		...
		Calendar argStart )
	{
		...
		try {
			...
			stmtDeleteByStartIdx.setString( argIdx++, CFIntDb2LUWSchema.getTimestampString( argStart ) );
		...
		Calendar argFinish )
	{
		...
		try {
			...
			if( argFinish != null ) {
				stmtDeleteByFinishIdx.setString( argIdx++, CFIntDb2LUWSchema.getTimestampString( argFinish ) );
			}
			else {
