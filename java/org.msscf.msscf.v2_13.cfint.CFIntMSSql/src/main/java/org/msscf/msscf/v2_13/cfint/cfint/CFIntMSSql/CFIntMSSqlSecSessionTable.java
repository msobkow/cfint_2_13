...
package org.msscf.msscf.v2_13.cfint.cfint.CFIntMSSql;
...
/*
 *	CFIntMSSqlSecSessionTable PostgreSQL Jdbc DbIO implementation
 *	for SecSession.
 */
public class CFIntMSSqlSecSessionTable
	implements ICFIntSecSessionTable
{
	...
	public CFIntMSSqlSecSessionTable( CFIntMSSqlSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFSecSecSessionBuff Buff )
	{
		...
		try {
			...
			stmtCreateByPKey.setString( argIdx++, CFIntMSSqlSchema.getTimestampString( Start ) );
			if( Finish != null ) {
				stmtCreateByPKey.setString( argIdx++, CFIntMSSqlSchema.getTimestampString( Finish ) );
			}
			else {
				stmtCreateByPKey.setNull( argIdx++, java.sql.Types.VARCHAR );
			}
	...
	throws SQLException
	{
		...
		int idxcol = 1;
		CFSecSecSessionBuff buff = schema.getFactorySecSession().newBuff();		buff.setRequiredSecSessionId( CFIntMSSqlSchema.convertUuidString( resultSet.getString( idxcol ) ) );
		idxcol++;
		...
		idxcol++;
		buff.setRequiredStart( CFIntMSSqlSchema.convertTimestampString( resultSet.getString( idxcol ) ) );
		idxcol++;
		{
			Calendar colVal = CFIntMSSqlSchema.convertTimestampString( resultSet.getString( idxcol ) );
			if( resultSet.wasNull() ) {
	...
	public CFSecSecSessionBuff readDerivedByIdIdx( CFSecAuthorization Authorization,
		UUID SecSessionId )
	{
		final String S_ProcName = "CFIntMSSqlSecSessionTable.readDerivedByIdIdx() ";
		CFSecSecSessionBuff buff;
		if( ! schema.isTransactionOpen() ) {
			throw new CFLibUsageException( getClass(),
				S_ProcName,
	...
	public CFSecSecSessionBuff readDerivedByStartIdx( CFSecAuthorization Authorization,
		...
		Calendar Start )
	{
		final String S_ProcName = "CFIntMSSqlSecSessionTable.readDerivedByStartIdx() ";
		CFSecSecSessionBuff buff;
		if( ! schema.isTransactionOpen() ) {
			throw new CFLibUsageException( getClass(),
				S_ProcName,
		...
		Calendar Start )
	{
		...
		try {
			...
			stmtReadBuffByStartIdx.setString( argIdx++, CFIntMSSqlSchema.getTimestampString( Start ) );
		...
		Calendar Finish )
	{
		...
		try {
			...
			if( Finish != null ) {
				stmtReadBuffByFinishIdx.setString( argIdx++, CFIntMSSqlSchema.getTimestampString( Finish ) );
			}
			else {
		...
		CFSecSecSessionBuff Buff )
	{
		...
		try {
			...
			stmtUpdateByPKey.setString( argIdx++, CFIntMSSqlSchema.getTimestampString( Start ) );
			if( Finish != null ) {
				stmtUpdateByPKey.setString( argIdx++, CFIntMSSqlSchema.getTimestampString( Finish ) );
			}
			else {
				stmtUpdateByPKey.setNull( argIdx++, java.sql.Types.VARCHAR );
			}
		...
		Calendar argStart )
	{
		...
		try {
			...
			stmtDeleteByStartIdx.setString( argIdx++, CFIntMSSqlSchema.getTimestampString( argStart ) );
			Object stuff = null;
		...
		Calendar argFinish )
	{
		...
		try {
			...
			if( argFinish != null ) {
				stmtDeleteByFinishIdx.setString( argIdx++, CFIntMSSqlSchema.getTimestampString( argFinish ) );
			}
			else {
