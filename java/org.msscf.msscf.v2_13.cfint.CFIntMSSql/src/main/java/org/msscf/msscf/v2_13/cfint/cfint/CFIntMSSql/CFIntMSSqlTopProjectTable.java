// Description: Java 11 MS SQL Server Developer Edition Jdbc DbIO implementation for TopProject.
...
/*
 *	CFIntMSSqlTopProjectTable PostgreSQL Jdbc DbIO implementation
 *	for TopProject.
 */
public class CFIntMSSqlTopProjectTable
	implements ICFIntTopProjectTable
{
	...
	public CFIntMSSqlTopProjectTable( CFIntMSSqlSchema argSchema ) {
		schema = argSchema;
	}
	...
	public CFIntTopProjectBuff readDerivedByIdIdx( CFSecAuthorization Authorization,
		...
		long Id )
	{
		final String S_ProcName = "CFIntMSSqlTopProjectTable.readDerivedByIdIdx() ";
		CFIntTopProjectBuff buff;
		if( ! schema.isTransactionOpen() ) {
			throw new CFLibUsageException( getClass(),
				S_ProcName,
	...
	public CFIntTopProjectBuff readDerivedByNameIdx( CFSecAuthorization Authorization,
		...
		String Name )
	{
		final String S_ProcName = "CFIntMSSqlTopProjectTable.readDerivedByNameIdx() ";
		CFIntTopProjectBuff buff;
		if( ! schema.isTransactionOpen() ) {
			throw new CFLibUsageException( getClass(),
				S_ProcName,
		...
		long TopDomainId )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			String sql = "{ call sp_read_tprjdef_by_topdomainidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ) }";
		...
		long argId )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			String sql = "exec sp_delete_tprjdef_by_ididx ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?";
		...
		long argTopDomainId )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			String sql = "exec sp_delete_tprjdef_by_topdomainidx ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?";
			...
			Object stuff = null;
			boolean moreResults = stmtDeleteByTopDomainIdx.execute();
			while( stuff == null ) {
				try {
					moreResults = stmtDeleteByTopDomainIdx.getMoreResults();
				}
				catch( SQLException e ) {
					throw new CFLibDbException( getClass(),
						S_ProcName,
				...
				if( moreResults ) {
					try {
						stuff = stmtDeleteByTopDomainIdx.getResultSet();
					}
					catch( SQLException e ) {
					}
				}
				else if( -1 == stmtDeleteByTopDomainIdx.getUpdateCount() ) {
					break;
				}
			}
		}
