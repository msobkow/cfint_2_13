...
/*
 *	CFIntPgSqlSecSessionTable PostgreSQL Jdbc DbIO implementation
 *	for SecSession.
 */
public class CFIntPgSqlSecSessionTable
	implements ICFIntSecSessionTable
{
	...
	public CFIntPgSqlSecSessionTable( CFIntPgSqlSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFSecSecSessionBuff Buff )
	{
		...
		try {
			...
			stmtCreateByPKey.setString( argIdx++, CFIntPgSqlSchema.getTimestampString( Start ) );
			if( Finish != null ) {
				stmtCreateByPKey.setString( argIdx++, CFIntPgSqlSchema.getTimestampString( Finish ) );
			}
			else {
				stmtCreateByPKey.setNull( argIdx++, java.sql.Types.VARCHAR );
			}
	...
	throws SQLException
	{
		...
		int idxcol = 1;
		CFSecSecSessionBuff buff = schema.getFactorySecSession().newBuff();		buff.setRequiredSecSessionId( CFIntPgSqlSchema.convertUuidString( resultSet.getString( idxcol ) ) );
		idxcol++;
		...
		idxcol++;
		buff.setRequiredStart( CFIntPgSqlSchema.convertTimestampString( resultSet.getString( idxcol ) ) );
		idxcol++;
		{
			Calendar colVal = CFIntPgSqlSchema.convertTimestampString( resultSet.getString( idxcol ) );
			if( resultSet.wasNull() ) {
		...
		idxcol++;
		{
			UUID colVal = CFIntPgSqlSchema.convertUuidString( resultSet.getString( idxcol ) );
			if( resultSet.wasNull() ) {
	...
	public CFSecSecSessionBuff readDerivedByIdIdx( CFSecAuthorization Authorization,
		UUID SecSessionId )
	{
		final String S_ProcName = "CFIntPgSqlSecSessionTable.readDerivedByIdIdx() ";
		CFSecSecSessionBuff buff;
		if( ! schema.isTransactionOpen() ) {
			throw new CFLibUsageException( getClass(),
				S_ProcName,
	...
	public CFSecSecSessionBuff readDerivedByStartIdx( CFSecAuthorization Authorization,
		...
		Calendar Start )
	{
		final String S_ProcName = "CFIntPgSqlSecSessionTable.readDerivedByStartIdx() ";
		CFSecSecSessionBuff buff;
		if( ! schema.isTransactionOpen() ) {
			throw new CFLibUsageException( getClass(),
				S_ProcName,
		...
		Calendar Start )
	{
		...
		try {
			...
			stmtReadBuffByStartIdx.setString( argIdx++, CFIntPgSqlSchema.getTimestampString( Start ) );
		...
		Calendar Finish )
	{
		...
		try {
			...
			if( Finish != null ) {
				stmtReadBuffByFinishIdx.setString( argIdx++, CFIntPgSqlSchema.getTimestampString( Finish ) );
			}
			else {
		...
		UUID priorSecSessionId )
	{
		...
		try {
			...
			if( Finish != null ) {
				stmtPageBuffByFinishIdx.setString( argIdx++, CFIntPgSqlSchema.getTimestampString( Finish ) );
			}
			else {
		...
		CFSecSecSessionBuff Buff )
	{
		...
		try {
			...
			stmtUpdateByPKey.setString( argIdx++, CFIntPgSqlSchema.getTimestampString( Start ) );
			if( Finish != null ) {
				stmtUpdateByPKey.setString( argIdx++, CFIntPgSqlSchema.getTimestampString( Finish ) );
			}
			else {
				stmtUpdateByPKey.setNull( argIdx++, java.sql.Types.VARCHAR );
			}
		...
		Calendar argStart )
	{
		...
		try {
				...
				stmtDeleteByStartIdx.setString( argIdx++, CFIntPgSqlSchema.getTimestampString( argStart ) );
		...
		Calendar argFinish )
	{
		...
		try {
				...
				if( argFinish != null ) {
					stmtDeleteByFinishIdx.setString( argIdx++, CFIntPgSqlSchema.getTimestampString( argFinish ) );
				}
				else {
