// Description: Java 11 XMsg Client DbIO implementation for CFInt.
...
public class CFIntXMsgClntSchema
	extends CFIntSchema
{
	protected CFTipClientHandler cftipClientHandler = null;
	protected boolean transactionOpen = false;

	public CFIntXMsgClntSchema() {
		super();
		getCFTipClientHandler();
		getResponseHandler();
		tableCluster = new CFIntXMsgClntCluster( this );
		tableHostNode = new CFIntXMsgClntHostNode( this );
		tableISOCcy = new CFIntXMsgClntISOCcy( this );
		tableISOCtry = new CFIntXMsgClntISOCtry( this );
		tableISOCtryCcy = new CFIntXMsgClntISOCtryCcy( this );
		tableISOCtryLang = new CFIntXMsgClntISOCtryLang( this );
		tableISOLang = new CFIntXMsgClntISOLang( this );
		tableISOTZone = new CFIntXMsgClntISOTZone( this );
		tableLicense = new CFIntXMsgClntLicense( this );
		tableMajorVersion = new CFIntXMsgClntMajorVersion( this );
		tableMimeType = new CFIntXMsgClntMimeType( this );
		tableMinorVersion = new CFIntXMsgClntMinorVersion( this );
		tableSecApp = new CFIntXMsgClntSecApp( this );
		tableSecDevice = new CFIntXMsgClntSecDevice( this );
		tableSecForm = new CFIntXMsgClntSecForm( this );
		tableSecGroup = new CFIntXMsgClntSecGroup( this );
		tableSecGroupForm = new CFIntXMsgClntSecGroupForm( this );
		tableSecGrpInc = new CFIntXMsgClntSecGrpInc( this );
		tableSecGrpMemb = new CFIntXMsgClntSecGrpMemb( this );
		tableSecSession = new CFIntXMsgClntSecSession( this );
		tableSecUser = new CFIntXMsgClntSecUser( this );
		tableService = new CFIntXMsgClntService( this );
		tableServiceType = new CFIntXMsgClntServiceType( this );
		tableSubProject = new CFIntXMsgClntSubProject( this );
		tableSysCluster = new CFIntXMsgClntSysCluster( this );
		tableTSecGroup = new CFIntXMsgClntTSecGroup( this );
		tableTSecGrpInc = new CFIntXMsgClntTSecGrpInc( this );
		tableTSecGrpMemb = new CFIntXMsgClntTSecGrpMemb( this );
		tableTenant = new CFIntXMsgClntTenant( this );
		tableTld = new CFIntXMsgClntTld( this );
		tableTopDomain = new CFIntXMsgClntTopDomain( this );
		tableTopProject = new CFIntXMsgClntTopProject( this );
		tableURLProtocol = new CFIntXMsgClntURLProtocol( this );
		setResponseHandlerSchemaObj( null );
	}

	public CFIntXMsgClntSchema( CFIntSchemaObj handlerSchema ) {
		super();
		getCFTipClientHandler();
		getResponseHandler();
		tableCluster = new CFIntXMsgClntCluster( this );
	...
	public CFIntXMsgClntSchema( CFIntSchemaObj handlerSchema, CFIntConfigurationFile conf ) {
		super( conf );
		getCFTipClientHandler();
		getResponseHandler();
		tableCluster = new CFIntXMsgClntCluster( this );
	...
	public CFIntXMsgClntSchema( CFIntSchemaObj handlerSchema, String argJndiName ) {
		super( argJndiName );
		getCFTipClientHandler();
		getResponseHandler();
		tableCluster = new CFIntXMsgClntCluster( this );
	...
	/**
	 *	Get the CFIntSchemaObj that is used by the response processor to
	 *	import the response objects during the parsing of response message strings.
	 *
	 *	@return	The CFIntSchemaObj that is providing a persistence implementation
	 *			for the parsing of response message strings.
	 */
	public ICFIntSchemaObj getResponseHandlerSchemaObj() {
		ICFIntSchemaObj handlerSchema = getResponseHandler().getSchemaObj();
		return( handlerSchema );
	}

	/**
	 *	Set the CFIntSchemaObj that is to be used by the response processor to
	 *	import the response objects during the parsing of response message strings.
	 *
	 *	@param	handlerSchema The CFIntSchemaObj that is to provide a persistence implementation
	 *			for the parsing of response message strings.
	 */
	public void setResponseHandlerSchemaObj( ICFIntSchemaObj handlerSchema ) {
	...
	/**
	 *	Get the CFIntDeleted flag that is used by the response processor to
	...
	/**
	 *	Set the CFIntDeleted flag that is to be used by the response processor to
	...
	public boolean isConnected() {
		ICFIntSchemaObj schemaObj = getResponseHandlerSchemaObj();
	...
	public CFIntXMsgRspn getResponseHandler() {
		CFIntXMsgRspn responseHandler = null;
		CFTipClientHandler clientHandler = getCFTipClientHandler();
		responseHandler = (CFIntXMsgRspn)clientHandler.getResponseHandler();
		if( responseHandler == null ) {
			responseHandler = new CFIntXMsgRspn();
	...
	public boolean connect( String loginId, String password, String clusterName, String tenantName ) {
		...
		try {
			...
			rqst = CFIntXMsgSchemaMessageFormatter.formatRqstXmlPreamble() + "\n"
				+	"\t" + CFIntXMsgSchemaMessageFormatter.formatRqstLogIn( "\n\t\t\t",
								loginId,
								deviceName,
								devEncPWHash,
								clusterName,
	...
	public void logout( CFSecAuthorization auth ) {
		...
		String rqst = CFIntXMsgSchemaMessageFormatter.formatRqstXmlPreamble() + "\n"
			+	"\t" + CFIntXMsgSchemaMessageFormatter.formatRqstLogOut( "\n\t\t\t",
		...
		String fileContent )
	{
		...
		String rqst = CFIntXMsgSchemaMessageFormatter.formatRqstXmlPreamble() + "\n"
			+	"\t" + CFIntXMsgSchemaMessageFormatter.formatRqstFileImport( "\n\t\t\t",
						fileName,
						fileContent ) + "\n"
			+	CFIntXMsgSchemaMessageFormatter.formatRqstXmlPostamble();
		try {
		...
		CFIntXMsgRspn responseParser = (CFIntXMsgRspn)responseHandler;
...
//						Syslog.warn("\t\t\tReplacing invalid character '" + c + "' with space");
	...
	public static String getNullableString(ResultSet reader, int colidx) {
		try {
			...
			throw new CFLibDbException( CFIntXMsgClntSchema.class,
				"getNullableString",
				e );
		}
	}
	...
	public static Calendar convertDateString(String val) {
		if( ( val == null ) || ( val.length() == 0 ) ) {
			...
			throw new CFLibUsageException( CFIntXMsgClntSchema.class,
				"convertDateString",
